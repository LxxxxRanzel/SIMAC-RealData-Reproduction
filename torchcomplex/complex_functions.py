...
"""
import torch
from torch.nn.functional import relu, max_pool2d, max_pool3d, avg_pool2d, avg_pool1d, dropout, dropout2d, dropout3d, interpolate, sigmoid, tanh, leaky_relu, max_pool1d
...
def complex_max_pool1d(input, kernel_size, stride=None, padding=0, dilation=1, ceil_mode=False, return_indices=False):
    '''
    Perform complex max pooling by selecting on the absolute value of the complex values.
    '''
    # Perform max pooling on the absolute value
    ...
    # Convert the absolute value back to complex type
    absolute_value = absolute_value.type(torch.complex64)

    # Retrieve the corresponding phase (angle) of the complex values using atan2
    angle = torch.atan2(input.imag, input.real)

    # Get only the phase values selected by max pool
    angle = _retrieve_elements_from_indices(angle, indices)

    # Return the complex number with the selected magnitude and phase
